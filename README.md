...
> The Solana smart wallet with a transaction firewall.
...
Built on the [Swig](https://onswig.com) smart-account protocol. Ships as a
...
Phantom shows a base58 program ID, you click **Confirm**, and the chain
...
The third row is the wedge. Coinbase x402 + PayAI are live on Solana, and
...
- **Connect approval** — first time a site connects, the user explicitly
  grants trust with a Phantom-style "Allow connection?" prompt. Per-origin
...
- **Wallet Standard implementation** — discovered automatically by any
  dApp using `@wallet-standard/app` or `@solana/wallet-adapter`. Exposes
...
- **`/scrybe`** — pay-per-question oracle on the **real x402 protocol**.
  ...
  header, the server forwards to [PayAI's](https://facilitator.payai.network)
  public devnet facilitator which co-signs as feePayer and lands the tx
  on-chain. Real settlement, real explorer link, ≈ 0.001 USDC per question.

### 3. The merchant + analyze server — `apps/server`

Fastify, devnet by default. Two surfaces in one process:

- `POST /v1/analyze` — pre-sign analyzer the extension and the showcase
  both call. Decodes the tx, hits Solana simulation, runs the policy DSL,
  returns structured findings + estimated changes.
- `GET /demo/scrybe` — merchant side of the x402 demo. Returns 402 with
  spec-compliant PaymentRequirements; on PAYMENT-SIGNATURE, calls PayAI's
  `/verify` then `/settle` and replies with the answer + on-chain proof.

A one-time CLI generates the merchant keypair, requests a devnet airdrop,
...
| Package | Role |
|---|---|
| `@blackthorn/swig-guard`      | Policy DSL + analyzer. Pre-sign rules, x402 rules, allowance rules, behavioral alerts. |
| `@blackthorn/ext-protocol`    | Type-safe message envelope shared by every extension surface. |
| `@blackthorn/wallet-adapter`  | Wallet Standard adapter the showcase consumes. |
| `@blackthorn/ui`              | Design tokens (`tokens.css`) — single source of truth for the monochrome white-on-black palette. |
| `@blackthorn/showcase-ui`     | Shared chrome for the showcase landing + sites. |
...
# 2. Bootstrap the x402 merchant on devnet (one-time)
pnpm --filter @deltag/server x402-setup
...
If devnet airdrop is rate-limited, the script prints the merchant
address; send ~0.05 devnet SOL there from any wallet, then rerun.
...
2. Hit **Airdrop** in the popup to fund the authority on devnet. For the
   x402 demo, grab USDC from <https://faucet.circle.com> (Solana / devnet).
...
4. Click **Connect Wallet** → BLACKTHORN appears at the top of the picker.
   The extension prompts you to allow the origin (Phantom-style).
...
pnpm --filter @deltag/server x402-setup   # bootstrap merchant on devnet
```
