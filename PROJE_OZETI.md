...
Solana'da bir işlem imzalanmadan önce otomatik olarak kontrol eden, "bu güvenli mi yoksa tehlikeli mi?" sorusuna gerekçeli cevap veren bir güvenlik altyapısı.

---

## Problem Ne?

Solana'da bir kullanıcı bir siteye girdiğinde "Approve" butonuna basar. Ama:
...
3. **ALT çözülür:** Bazı modern Solana işlemlerinde adresler "Lookup Table" içinde saklanır. Bunlar RPC'den çekilerek tamamlanır.
4. **Hesap durumları alınır:** İşlemdeki tüm hesapların mevcut bakiyeleri RPC'den çekilir.
5. **Simülasyon çalışır:** "Bu işlem gerçek olsaydı ne olurdu?" sorusu Solana RPC'ye sorulur. İmza gerekmez.
6. **Bakiye değişimleri hesaplanır:** SOL ve token değişimleri simülasyon öncesi/sonrası karşılaştırılarak bulunur.
...
Solana'nın yeni token standardı (Token-2022), tehlikeli ekler içerebilir:
...
| `maxLossPercent` | SOL kaybı bu yüzdeyi aşarsa blokla |
...
- **DeFi Permissive (Esnek):** Sadece bilinen kötü aktörler varsa blokla; SOL kaybı %50'yi aşarsa uyar.
...
`apps/showcase` klasöründe 5 ayrı "sahte site" var. Her biri gerçek bir Solana tehdit senaryosunu canlandırıyor. Bunları açıp Blackthorn'un tehdidi nasıl yakaladığını görebilirsin.
...
3. **Senaryo üret** — Test için gerçek bir Solana işlemi hazırla
...
`packages/browser-extension/` — Herhangi bir Solana dApp'inde çalışır. Kurulumdan sonra:
...
3. Kullanıcı Solana üzerinden ödeme yapar (USDC cinsinden, istek başı ~$0.001)
...
- Sadece VersionedTransaction formatı destekleniyor (legacy format desteklenmiyor).
...
| `X402_PAY_TO` | Ödemelerin gideceği Solana adresi |
