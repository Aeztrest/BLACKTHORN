...
import type { AnalysisResult, Cluster } from "./types";
...
export async function analyzeTransaction(
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === "AbortError") {
      throw new AnalyzeError(`BLACKTHORN analyze timed out after ${cfg.timeoutMs ?? DEFAULT_TIMEOUT}ms`, undefined, undefined, err);
    }
    const msg = err instanceof Error ? err.message : String(err);
    throw new AnalyzeError(`BLACKTHORN analyze request failed: ${msg}`, undefined, undefined, err);
  } finally {
    clearTimeout(t);
  }
}
