...
/* ────────────────────────────────────────────────────────────────────────── */
/* 2b. Pre-sign analysis result (mirrors @blackthorn/swig-guard AnalysisResult) */
/* ────────────────────────────────────────────────────────────────────────── */
...
export interface SolDeltaPayload {
  account: string;
...
export interface AnalyzeResponse {
  ...
  estimatedChanges: {
    sol: SolDeltaPayload[];
    tokens: TokenDeltaPayload[];
    approvals: ApprovalPayload[];
    delegates: ApprovalPayload[];
  };
...
export interface ExtRpc {
  ...
  "wallet.create":     { req: { passphrase: string; network: Cluster };   rsp: { walletAddress: string; authorityAddress: string } };
  ...
  "wallet.airdrop":    { req: void;                                       rsp: { signature: string; amountSol: number } };
  "wallet.provisionSwig": { req: void;                                    rsp: { swigAddress: string; walletAddress: string; alreadyOnChain: boolean } };
  "wallet.balance":    { req: { address?: string };                       rsp: { lamports: number } };
  /** User-initiated SOL transfer from the authority key.
   *  Builds + signs + broadcasts a SystemProgram.transfer locally; the
   *  popup never sees the private key. */
  "wallet.transferSol": { req: { to: string; amountSol: number };          rsp: { signature: string } };

  /* Sign + tx ────────────────────────────── */
  "tx.sign":           { req: { requestId: string; accept: boolean };     rsp: { signed?: string; signature?: string; rejection?: string } };
  "tx.send":           { req: { txBase64: string };                       rsp: { signature: string } };
  "tx.peekRequest":    { req: void;                                       rsp: { requestId: string; kind: "message" | "transaction" | "transactionAndSend"; origin: string; payloadBase64: string } | null };
  ...
  /* Network ──────────────────────────────── */
  "network.set":       { req: { cluster: Cluster };                       rsp: { ok: true } };
}
...
/**
 * Wallet Standard / Solana provider methods, forwarded from the page through
 * the content script's `bx-wallet-standard` port.
 */
export interface ExtWalletStandardMethods {
  "ws.connect":               { req: { origin: string };                                     rsp: { walletAddress: string; authorityAddress: string } };
  "ws.disconnect":            { req: { origin: string };                                     rsp: { ok: true } };
  "ws.signTransaction":       { req: { origin: string; txBase64: string };                   rsp: { signedTxBase64: string } };
  "ws.signAndSendTransaction": { req: { origin: string; txBase64: string };                  rsp: { signedTxBase64: string; signature: string } };
  "ws.signMessage":           { req: { origin: string; messageBase64: string };              rsp: { signatureBase64: string } };
}
