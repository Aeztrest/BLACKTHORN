/**
 * Scrybe — pay-per-question oracle, x402 over Solana devnet.
 *
...
import {
  ...
  Loader2, Send, ChevronRight, ChevronDown, Lock,
} from "lucide-react";
import { PublicKey } from "@solana/web3.js";
import { useWallet } from "../../wallet/context";
import {
  buildX402PaymentTx, encodePaymentHeader, type PaymentRequirements,
} from "./build-x402";
...
export default function Scrybe() {
  ...
  async function runPayment(q: string) {
    ...
    try {
      ...
      const built = await buildX402PaymentTx(userPubkey, requirements);

      // 4. Ask wallet to sign — BLACKTHORN runs pre-sign analysis here
      update({ phase: "signing" });
      const signed = await adapter.signTransaction(built.tx);

      // 5. Replay with the signed payload in PAYMENT-SIGNATURE
      update({ phase: "settling" });
      const headerValue = encodePaymentHeader(requirements, signed);
          ...
          <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-7">
            <div>
              ...
              <p className="text-white/50 mt-3 leading-relaxed max-w-xl">
                Pay-per-question oracle running the HTTP&nbsp;402 protocol on Solana devnet.
...
function ProgressStep({ entry }: { entry: AnswerEntry }) {
  const PHASES: Array<{ key: Phase; label: string }> = [
    ...
    { key: "settling",  label: "Settling on Solana" },
  ];
...
function SettlementReceipt({ signature, payer, network, elapsedMs }: {
  ...
  const cluster = network?.includes("devnet") ? "devnet"
                : network?.includes("testnet") ? "testnet" : "mainnet-beta";
      ...
      {open && (
        <div className="px-4 pb-4 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
          {[
            ...
            { n: "04", t: "Settle",  b: "PayAI broadcasts on devnet" },
          ].map((s) => (
...
function friendlyError(msg: string): string {
  if (msg.toLowerCase().includes("insufficient")) {
    return "Your wallet doesn't have enough devnet USDC. Get some from faucet.circle.com (Solana, devnet).";
  }
  ...
  if (msg.toLowerCase().includes("no sign") || msg.includes("NO_SIGN_TRANSACTION")) {
    return "Your wallet doesn't support partial signing. Reconnect with BLACKTHORN or Phantom.";
  }
  return msg;
}
