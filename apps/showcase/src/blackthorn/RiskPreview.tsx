...
export function RiskPreview({
  open, tx, userWallet, scenarioLabel,
  ...
  useEffect(() => {
    if (!open || !tx || !userWallet) return;
    let cancelled = false;
    setResult(null); setError(null); setLoading(true);
    analyzeTransactionForPreview(tx, userWallet, { cluster: "devnet" })
...
function Changes({ result }: { result: AnalysisResult }) {
  const sol = result.estimatedChanges.sol.filter((s) => s.deltaLamports !== null && s.deltaLamports !== 0);
  const tokens = result.estimatedChanges.tokens;
  const approvals = result.estimatedChanges.approvals;
  if (!sol.length && !tokens.length && !approvals.length) return null;
      ...
      <div className="space-y-1.5 text-[11px]">
        ...
        {tokens.map((t, i) => (
          <DeltaRow key={`tok-${i}`}
            ...
            value={t.deltaAmount ?? "—"}
            negative={t.deltaAmount?.startsWith("-") ?? false}
          />
        ))}
        {approvals.map((a, i) => (
          <DeltaRow key={`apv-${i}`}
            label={`Approve → ${shortAddr(a.delegate)}`}
