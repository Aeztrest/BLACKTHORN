...
import {
  ...
  Github, Wallet, Sparkles, Layers, Radar, BookOpen, Cpu,
  Network, FileSearch, BellRing, KeyRound, Gauge,
  XCircle, Menu, X as XIcon,
} from "lucide-react";

const SOCIAL_GITHUB = "https://github.com/Aeztrest/DeltaProtokol";
const SOCIAL_X = "https://x.com/blackthornxyz";
        ...
        <nav className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map((l) => (
            <Link
              key={l.label}
              to={l.to}
              className="px-3.5 py-2 rounded-lg text-sm text-white/65 hover:text-white hover:bg-white/[0.04] transition-colors"
            >
              {l.label}
            </Link>
          ))}
...
function Logo() {
  return (
    <span className="relative w-8 h-8 grid place-items-center rounded-lg border border-white/15 bg-white/5">
      <Shield size={14} className="text-white" strokeWidth={2.4} />
    ...
    <section ref={ref} className="relative pt-36 pb-24 px-6 overflow-hidden">
      <motion.div style={{ y, opacity }} className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-12 items-center relative">
        <div className="lg:col-span-7">
          ...
          >
            BLACKTHORN is a Solana smart wallet that simulates every transaction,
...
function LiveAnalysisCard() {
  const findings = [
    { sev: "danger", icon: AlertTriangle, label: "Transfers SOL to unknown wallet", detail: "10.0 SOL → 3kBz…q9F" },
...
function ProblemSolution() {
  const rows = [
    {
      moment: "Blind sign",
      old:    "Phantom shows a base58 program ID and a Confirm button.",
...
function ShowcaseStrip() {
  return (
    <Section
      ...
      action={{ label: "Open showcase hub", to: "/" }}
    >
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/40 mt-0.5">The Solana wallet that watches what happens after you sign.</p>
          </div>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <Link to="/" className="text-xs text-white/55 hover:text-white px-3 py-1.5 rounded-md hover:bg-white/[0.04]">Showcase</Link>
          <Link to="/docs" className="text-xs text-white/55 hover:text-white px-3 py-1.5 rounded-md hover:bg-white/[0.04]">Docs</Link>
          <Link to="/install" className="text-xs text-white/55 hover:text-white px-3 py-1.5 rounded-md hover:bg-white/[0.04]">Install</Link>
