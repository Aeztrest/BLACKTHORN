...
import { POLICY_TEMPLATES, type PolicyTemplateId } from "@blackthorn/swig-guard";
import { Mark } from "@blackthorn/ui";
...
export function Onboarding() {
  ...
  useEffect(() => {
    ...
    const fetchBal = async () => {
      try {
        ...
        setAuthorityBalance(res.lamports / LAMPORTS_PER_SOL);
      } catch { /* ignore */ }
    };
    void fetchBal();
  ...
  const onCreateWallet = async () => {
    setError(null);
    try {
      const res = await rpc.call("wallet.create", { passphrase, network: "devnet" });
  ...
  const onProvision = async () => {
    ...
    setError(null);
    setProvisionMsg("Submitting create-Swig instruction…");
    try {
      const res = await rpc.call("wallet.provisionSwig", undefined as never);
      setWalletAddress(res.walletAddress);
