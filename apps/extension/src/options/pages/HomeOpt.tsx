 ...
 * Spec: docs/wallet-spec.md §7.2.
 *
 * Live-polls authority + smart-wallet balances every 8 seconds. Until the
 * Swig PDA is provisioned, the authority key IS the wallet — we surface its
 * balance in the hero so the user always sees a real number.
 */

import { useCallback, useEffect, useState } from "react";
import { Link } from "react-router-dom";
import {
  Send, Download, Sparkles, Loader2, ExternalLink, Shield, Clock, ArrowRight,
} from "lucide-react";
import { useRpc, useWalletState } from "../../shared/state-context";
import { LAMPORTS_PER_SOL } from "@solana/web3.js";
import type { GuardPolicy } from "@blackthorn/swig-guard";
import { OptionsSendModal, OptionsReceiveModal } from "../components/SendReceiveModal";
...
export function HomeOpt() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      if (state.walletAddress) {
        const r = await rpc.call("wallet.balance", { address: state.walletAddress });
        setWalletBal(r.lamports / LAMPORTS_PER_SOL);
      } else {
        setWalletBal(null);
      }
      if (state.authorityAddress) {
        const r = await rpc.call("wallet.balance", { address: state.authorityAddress });
        setAuthBal(r.lamports / LAMPORTS_PER_SOL);
      }
    } catch { /* ignore — UI shows last known */ }
  }, [state, rpc]);
  ...
  useEffect(() => {
    void rpc.call("policy.read", undefined as never).then((p) => setPolicy(p as GuardPolicy));
  }, [rpc]);
  ...
  // Hero balance reflects what the user can actually spend right now.
  // Before Swig is provisioned, the authority IS the wallet.
  const heroBalance = state.walletAddress ? walletBal : authBal;
  const heroAddress = state.walletAddress ?? state.authorityAddress;
  const heroLabel = state.walletAddress ? "Smart wallet" : "Authority key (smart wallet pending)";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-text-muted text-sm mt-1">
          Your wallet is live on {state.network}. Every transaction passes BLACKTHORN before signing.
        </p>
      </div>
      ...
      >
        <p className="label">{heroLabel} · {shortAddr(heroAddress)}</p>
        <p className="text-5xl font-extrabold leading-none font-mono tracking-tight">
          {heroBalance === null ? "—" : heroBalance.toFixed(4)}
          <span className="text-2xl text-text-faint font-bold ml-2">SOL</span>
        </p>
        <p className="text-text-faint text-xs mt-2">
          {state.walletAddress
            ? (walletBal && walletBal > 0
                ? "Funds available in your Swig PDA."
                : "Smart wallet empty — receive SOL or move some from your authority.")
            : "Provision the Swig PDA from Settings to upgrade to the smart wallet (gasless sub-keys, x402 allowances)."}
        </p>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          <button onClick={onAirdrop} disabled={airdropping} className="btn-ghost">
            {airdropping
              ...
              : <><Sparkles size={13} /> Devnet airdrop</>}
          </button>
        </div>

        {airdropMsg && (
          <p className="text-[11px] mt-3" style={{ color: "var(--ok)" }}>{airdropMsg}</p>
        )}
        {airdropError && (
          <p className="text-[11px] mt-3" style={{ color: "var(--bad)" }}>{airdropError}</p>
        )}
      </section>

      {/* Two-up: policy summary + connected sites */}
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <Link to="/sites" className="card hover:bg-white/[0.025] transition-colors">
          ...
          <p className="text-text-faint text-xs leading-relaxed">
            Every dApp you connect, every x402 paywall you visit. Per-origin caps, pause, revoke.
          </p>
        </Link>
      </div>

      {/* Authority detail (only meaningful once Swig provisioned) */}
      {state.walletAddress && (
        <section className="card">
          <div className="flex items-center justify-between gap-4 mb-3">
            <h2 className="font-bold text-sm">Authority key</h2>
            <a href={explorerAddress(state.authorityAddress, state.network)} target="_blank" rel="noreferrer"
               className="text-xs text-accent-soft hover:text-text inline-flex items-center gap-1">
              View on Explorer <ExternalLink size={11} />
            </a>
          </div>
          <p className="font-mono text-xs text-text-muted break-all mb-2">{state.authorityAddress}</p>
          <p className="text-text-faint text-xs">
            Pays fees + signs Swig instructions on your behalf. Balance: <span className="font-mono text-text-muted">{authBal === null ? "—" : `${authBal.toFixed(4)} SOL`}</span>.
          </p>
        </section>
      )}
...
function PolicySummary({ policy }: { policy: GuardPolicy | null }) {
  ...
  const rows: Array<[string, string]> = [
    ["Max loss per tx",     policy.maxLossPercent != null ? `${policy.maxLossPercent}%` : "—"],
    ["Block risky programs", policy.blockRiskyPrograms ? "On" : "Off"],
    ["Block new approvals",  policy.blockApprovalChanges ? "On" : "Off"],
    ["Require sim success",  policy.requireSuccessfulSimulation !== false ? "Yes" : "No"],
    ["x402 hourly cap",      policy.x402HourlyCap != null ? `$${policy.x402HourlyCap.toFixed(2)}` : "—"],
  ];
  return (
