...
export interface KeystoreRow {
  id: "primary";
  blob: EncryptedBlob;
  /** Public authority pubkey (base58). Stored alongside so the wallet UI can
   *  show an address even while locked. */
  authorityPubkey: string;
  /** 32-byte Swig id, base64. Persisted for PDA derivation while locked. */
  swigIdB64: string;
  createdAt: number;
}
