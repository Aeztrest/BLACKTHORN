 ...
 *
 * Spec: docs/extension-architecture.md §3 (background) + docs/x402-defense.md.
 */
...
export async function analyzeTransaction(
  ...
  try {
    ...
    if (!res.ok) {
      // Server reachable but rejected; treat as offline-with-context.
...
function normalize(d: ServerDecision): AnalyzeResponse {
  // Decision tier: if server flags unsafe → block.
  // If safe but findings have medium+ severity → advisory.
  // Otherwise allow.
  const findings = d.riskFindings ?? [];
  const top = topSeverity(findings);
  let decision: AnalyzeResponse["decision"];
  if (!d.safe) decision = "block";
  else if (top === "medium" || top === "high" || top === "critical") decision = "advisory";
  else decision = "allow";
  ...
  return {
    ...
    estimatedChanges: d.estimatedChanges ?? { sol: [], tokens: [], approvals: [], delegates: [] },
    simulationWarnings: d.simulationWarnings ?? [],
    offline: false,
  };
}

function topSeverity(findings: RiskFindingPayload[]): RiskFindingPayload["severity"] | null {
  let best: RiskFindingPayload["severity"] | null = null;
  let rank = 0;
  for (const f of findings) {
    const r = SEVERITY_ORDER[f.severity] ?? 0;
    if (r > rank) { rank = r; best = f.severity; }
  }
  return best;
}
...
function offlineResponse(message: string): AnalyzeResponse {
  return {
    ...
    riskFindings: [{
      ...
      severity: "medium",
      message: "BLACKTHORN's analyze server didn't respond. Sign only if you trust this dApp.",
    }],
