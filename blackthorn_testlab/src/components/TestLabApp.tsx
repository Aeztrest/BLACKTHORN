...
export default function TestLabApp() {
  ...
  const [swigApiKey, setSwigApiKey] = useState("");
  const [swigPolicyId, setSwigPolicyId] = useState("");
  const [swigAuthorityAddress, setSwigAuthorityAddress] = useState("");
  const [swigIdInput, setSwigIdInput] = useState("");
  const [swigPaymasterPubkey, setSwigPaymasterPubkey] = useState("");
  const [swigNetwork, setSwigNetwork] = useState<"devnet" | "mainnet">("devnet");
  ...
  const [swigPolicy, setSwigPolicy] = useState<SwigPolicy | null>(null);
  const [swigWalletResult, setSwigWalletResult] = useState<SwigWalletResult | null>(null);
  const [swigFinalizeSignature, setSwigFinalizeSignature] = useState("");
  ...
  async function onConnectWallet() {
    await runWithBusy("wallet-connect", async () => {
      ...
      if (option.kind === "standard") {
        ...
        setSwigAuthorityAddress((current) => current || account.address);
      } else {
        ...
        setSwigAuthorityAddress((current) => current || address);
      }
  ...
  function resolveSwigAuthorityAddress(): string {
    const address = swigAuthorityAddress.trim() || connectedWallet?.address || "";
    if (!address) {
      throw new Error("Swig authority icin signer wallet adresi gerekli.");
    }
    return address;
  }

  async function requestSwig<T>(body: Record<string, unknown>): Promise<T> {
    const response = await fetch("/api/swig", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (!response.ok) {
      throw new Error(data.message || data.error || "Swig istegi basarisiz.");
    }
    return data;
  }

  async function onUseConnectedWalletForSwig() {
    await runWithBusy("swig-use-wallet", async () => {
      const walletState = requireConnectedWallet();
      setSwigAuthorityAddress(walletState.address);
    });
  }

  async function onLoadSwigPolicy() {
    await runWithBusy("swig-policy", async () => {
      if (!swigPolicyId.trim()) throw new Error("Policy ID gerekli.");
      const response = await requestSwig<{ ok: true; policy: SwigPolicy }>({
        action: "getPolicy",
        policyId: swigPolicyId.trim(),
        apiKey: swigApiKey.trim() || undefined,
      });
      setSwigPolicy(response.policy);
    });
  }

  async function onCreateSwigWallet() {
    await runWithBusy("swig-create", async () => {
      if (!swigPolicyId.trim()) throw new Error("Policy ID gerekli.");
      const authorityAddress = resolveSwigAuthorityAddress();
      const response = await requestSwig<SwigWalletResult>({
        action: "createWallet",
        policyId: swigPolicyId.trim(),
        walletAddress: authorityAddress,
        network: swigNetwork,
        swigId: swigIdInput.trim() || undefined,
        paymasterPubkey: swigPaymasterPubkey.trim() || undefined,
        apiKey: swigApiKey.trim() || undefined,
      });

      setSwigWalletResult(response);
      setSwigFinalizeSignature("");

      if (response.mode === "unsigned") {
        setTxBase64(response.transactionBase64);
        setBatchInput(response.transactionBase64);
        setScenarioMeta({
          type: "",
          note: `Swig create wallet unsigned tx hazirlandi. swigId=${response.swigId}`,
          payer: authorityAddress,
          recipient: response.swigAddress,
          walletName: "Swig Smart Wallet",
        });
      }
    });
  }

  async function onFinalizeSwigWalletCreation() {
    await runWithBusy("swig-finalize", async () => {
      if (!swigWalletResult || swigWalletResult.mode !== "unsigned") {
        throw new Error("Once unsigned Swig wallet create adimini calistir.");
      }
      if (!connectedWallet?.address) {
        throw new Error("Unsigned create tx'i gondermek icin signer wallet bagli olmali.");
      }
      if (connectedWallet.address !== resolveSwigAuthorityAddress()) {
        throw new Error("Bagli wallet adresi, Swig authority adresi ile ayni olmali.");
      }

      const conn = new Connection(getRpcUrl(cluster), "confirmed");
      const tx = VersionedTransaction.deserialize(fromBase64(swigWalletResult.transactionBase64));
      const signedTx = await signWithConnectedWallet(tx);
      const signature = await conn.sendRawTransaction(signedTx.serialize(), { skipPreflight: false });
      await conn.confirmTransaction(signature, "confirmed");

      const base64 = await fetchBase64FromSignature(signature, getRpcUrl(cluster));
      setSwigFinalizeSignature(signature);
      setTxSignature(signature);
      setTxBase64(base64);
      setBatchInput(base64);
      setScenarioMeta({
        type: "signature",
        note: `Swig smart wallet create tx gonderildi. swigId=${swigWalletResult.swigId}`,
        signature,
        payer: connectedWallet.address,
        recipient: swigWalletResult.swigAddress,
        walletName: "Swig Smart Wallet",
      });
    });
  }
  ...
  const pages: Array<{
    ...
    },
    {
      id: "swig",
      title: "2. Swig Wallet",
      hint: "Policy + smart wallet",
      done: !!swigPolicy || !!swigWalletResult,
    },
    {
      id: "scenario",
      title: "3. Senaryo uret",
      hint: "Gercek tx / signature",
      done: !!txBase64,
    },
    {
      id: "analyze",
  ...
  function renderPage() {
    if (activePage === "welcome") {
      return (
        <section className="page-card">
          <h1>Blackthorn TestLab</h1>
          <p>
            Bu aktif Blackthorn test ortami. Adim adim ilerleyen bu laboratuvar ekraninda, yazilim
            bilmeyen biri bile soldaki adimlari sirayla ilerleyerek "normalde ne olur, Blackthorn nasil fark
          ...
          <div className="tips">
            ...
            <p>
              Aktif kaynak dizin: <code>/home/ubuntu/DeltaProtokol/blackthorn_testlab</code>
            </p>
            <ol>
              <li>Soldan "1. Ortam kontrol" adimina git.</li>
              <li>Sonra "2. Swig Wallet" asamasinda signer ve policy akisini kur.</li>
              <li>Ardindan "3. Senaryo uret" sayfasinda analyze icin tx verisini hazirla.</li>
            </ol>
          </div>
        </section>
      );
          ...
          <div className="form-grid two">
            <label>
              ...
              <select
                value={cluster}
                onChange={(e) => {
                  const nextCluster = e.target.value as Cluster;
                  setCluster(nextCluster);
                  if (nextCluster === "devnet") setSwigNetwork("devnet");
                  if (nextCluster === "mainnet-beta") setSwigNetwork("mainnet");
                }}
              >
                {CLUSTERS.map((c) => (
    ...
    if (activePage === "swig") {
      return (
        <section className="page-card">
          <h1>2) Swig Wallet Asamasi</h1>
          <p>
            Bu asama Swig smart wallet akisi icin ayrildi. Resmi Swig Portal API uzerinden policy
            kontrolu yapar, signer adresine bagli bir Swig wallet create transaction'i uretir ve
            isterse ayni sayfadan finalize eder.
          </p>
          <div className="info-banner">
            <strong>Guvenlik notu:</strong> Swig API key browser bundle icine gomulmuyor. Bu sayfa
            istegi Next.js server route'u uzerinden gonderiyor. `SWIG_API_KEY` env varsa onu,
            yoksa asagidaki gecici input'u kullanir.
          </div>
          <div className="wallet-panel">
            <div className="wallet-panel-header">
              <div>
                <h3>Signer Wallet Baglantisi</h3>
                <p>
                  Swig create tx'i unsigned donerse, finalize etmek icin authority signer wallet
                  bagli olmali.
                </p>
              </div>
          ...
          <div className="wallet-panel">
            <div className="wallet-panel-header">
              <div>
                <h3>Signer Test Transferi</h3>
                <p>
                  Swig tarafina gecmeden once signer wallet'in devnet uzerinde imza ve gonderim
                  akisini test edebilirsin.
                </p>
              </div>
            </div>
            <div className="form-grid two">
          ...
          <div className="form-grid two">
            <label>
              Swig API Key (opsiyonel, env yoksa kullan)
              <input
                type="password"
                value={swigApiKey}
                onChange={(e) => setSwigApiKey(e.target.value)}
                placeholder="sk_..."
              />
            </label>
            <label>
              ...
              <input
                value={swigPolicyId}
                onChange={(e) => setSwigPolicyId(e.target.value)}
                placeholder="Portal policy id"
              />
            </label>
          </div>
          <div className="form-grid two">
            <label>
              Signer / authority adresi
              <input
                value={swigAuthorityAddress}
                onChange={(e) => setSwigAuthorityAddress(e.target.value)}
                placeholder="Ed25519 Solana public key"
              />
            </label>
            <label>
              Swig network
              <select
                value={swigNetwork}
                onChange={(e) => setSwigNetwork(e.target.value as "devnet" | "mainnet")}
              >
          ...
          <div className="form-grid two">
            <label>
              ...
              <input
                value={swigIdInput}
                onChange={(e) => setSwigIdInput(e.target.value)}
                placeholder="Bos birakirsan portal uretir"
              />
            </label>
            <label>
              Paymaster pubkey (opsiyonel)
              <input
                value={swigPaymasterPubkey}
                onChange={(e) => setSwigPaymasterPubkey(e.target.value)}
                placeholder="Varsa create tx'i sponsorlar"
              />
            </label>
          </div>
          <div className="actions">
            <button onClick={onUseConnectedWalletForSwig} disabled={!!isBusy || !connectedWallet}>
              Bagli Wallet Adresini Kullan
            </button>
            <button onClick={onLoadSwigPolicy} disabled={!!isBusy}>
              Policy Bilgisini Getir
            </button>
            <button onClick={onCreateSwigWallet} disabled={!!isBusy}>
              Swig Wallet Create Tx Uret
            </button>
            <button
              onClick={onFinalizeSwigWalletCreation}
              disabled={!!isBusy || !swigWalletResult || swigWalletResult.mode !== "unsigned"}
            >
              Create Tx'i Imzala ve Gonder
            </button>
          </div>
          {connectedWallet ? (
            <div className="note">
              <h3>Bagli signer wallet</h3>
              <pre>{pretty({ wallet: connectedWallet.walletName, address: connectedWallet.address })}</pre>
            </div>
          ) : null}
          {swigPolicy ? (
            <div className="note">
              <h3>Swig Policy</h3>
              <pre>{pretty(swigPolicy)}</pre>
            </div>
          ) : null}
          {swigWalletResult ? (
            <div className="note">
              <h3>Swig Wallet Sonucu</h3>
              <pre>{pretty(swigWalletResult)}</pre>
            </div>
          ) : null}
          {swigFinalizeSignature ? (
            <div className="info-banner">
              <strong>Finalize signature:</strong> <code>{swigFinalizeSignature}</code>
            </div>
          ) : null}
        </section>
      );
    ...
    if (activePage === "scenario") {
      return (
        <section className="page-card">
          <h1>3) Gercek Hayat Senaryosu Olusturma</h1>
          <p>
            Bu sayfa transaction verisini hazirlar. Swig smart wallet kurulumu artik ayri
            asamaya tasindi; burada sadece analyze icin tx veya signature toplarsin.
          </p>
          <div className="actions">
    ...
    if (activePage === "mcp") {
      return (
        <section className="page-card">
          <h1>8) MCP Sayfasi</h1>
          <p>
            MCP, ajan/yardimci uygulamalarin Blackthorn ile konusmasini saglar. Bu sayfada agent
